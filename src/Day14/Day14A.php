...
class Day14A
{
    ...
    public function run(string $input): void
    {
        ...
        foreach ($robots as $robot) {
            if ($this->InBounds($robot[1], $robot[0], 0, 0, $ym - 1, $xm - 1)) {
                $quadrants['q1'] = ($quadrants['q1'] ?? 0) + 1;
            } elseif ($this->InBounds($robot[1], $robot[0], 0, $xm + 1, $ym - 1, $CUBOUND - 1)) {
                $quadrants['q2'] = ($quadrants['q2'] ?? 0) + 1;
            } elseif ($this->InBounds($robot[1], $robot[0], $ym + 1, 0, $RUBOUND - 1, $xm - 1)) {
                $quadrants['q3'] = ($quadrants['q3'] ?? 0) + 1;
            } elseif ($this->InBounds($robot[1], $robot[0], $ym + 1, $xm + 1, $RUBOUND - 1, $CUBOUND - 1)) {
